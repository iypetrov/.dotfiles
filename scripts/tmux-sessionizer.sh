...
target="$(find "${XDG_DOCUMENTS_DIR}/projects/common" "${XDG_DOCUMENTS_DIR}/projects/personal" "${XDG_DOCUMENTS_DIR}/projects/ip812" "${XDG_DOCUMENTS_DIR}/projects/work/cpx" -mindepth 1 -maxdepth 1 -type d | fzf)"
if [[ -z $"{target}" ]]; then
    exit 0
fi
